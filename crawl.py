...
class VnExpressCrawler:
    ...
    def __init__(self, base_url: str, output_file: str = "data.csv", log_file: str = "crawl.log"):
    ...
    def setup_logging(self) -> None:
        ...
        logging.basicConfig(
            ...
            handlers=[logging.FileHandler(self.log_file, mode="w", encoding="utf-8"), logging.StreamHandler()],
        )
    ...
    def load_page_with_retry(self) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                self.logger.info(f"Loading page (attempt {attempt + 1}/{self.max_retries}): {self.base_url}")
    ...
    def find_data_embed(self) -> Optional[object]:
        ...
        try:
            flourish_embeds = self.driver.find_elements(By.CSS_SELECTOR, ".flourish-embed")
    ...
    def test_embed_for_data(self, embed) -> bool:
        ...
        try:
            ...
            if tables:
                ...
                self.logger.debug(f"Embed test: {len(tables)} tables, {data_rows} data rows")
    ...
    def go_to_next_page(self) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                next_button = None
                next_selectors = ["button.next", "button[aria-label='Next page']", "button[title='Next page']"]
    ...
    def save_progress_to_csv(self, all_data: List[Tuple[str, str, str]]) -> None:
        ...
        try:
            with open(self.output_file, "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(["Tỉnh", "Phường, xã mới", "Phường, xã trước sáp nhập"])
        ...
        try:
            ...
            while current_page <= max_pages:
                ...
                if page_data:
                    ...
                    consecutive_empty_pages = 0
                    self.logger.info(
                        f"Extracted {len(page_data)} rows from page {current_page} (Total: {len(all_data)})"
                    )
                    ...
                    self.logger.warning(
                        f"No data found on page {current_page} ({consecutive_empty_pages} consecutive empty)"
                    )
