...
const provinceCoordinates = {
    ...
    'Quảng Trị': [16.8169, 107.1000],
    'Sóc Trăng': [9.6004, 105.9800],
    'Sơn La': [21.3256, 103.9188],
    'Tây Ninh': [11.3135, 106.0980],
...
function updateStatistics() {
    document.getElementById('total-units').textContent = data.statistics.total_units.toLocaleString('vi-VN');
    document.getElementById('total-provinces').textContent = data.statistics.total_provinces;
    document.getElementById('merged-units').textContent = data.statistics.total_merged.toLocaleString('vi-VN');
    document.getElementById('unchanged-units').textContent = data.statistics.total_unchanged;
}
...
function addProvinceMarkers() {
    ...
    function addBatch() {
        ...
        for (let i = 0; i < batchSize && index < data.provinces.length; i++, index++) {
...
function initCharts() {
    ...
    const topProvincesCtx = document.getElementById('top-provinces-chart').getContext('2d');
    ...
    const mergerLabels = Object.keys(mergerData).sort((a, b) => parseInt(a) - parseInt(b));
...
function performSearch(query) {
    ...
    const results = searchIndex
        ...
        .map(item => {
            ...
            const relevance = unitNewName.startsWith(normalizedQuery) ? 2 : 
                             unitNewName.includes(normalizedQuery) ? 1 : 0;
            return {
                type: 'unit',
                data: item.unit,
                relevance: relevance
        ...
        results.forEach(result => {
            ...
            if (result.type === 'unit') {
                ...
                item.addEventListener('click', () => {
                    const province = data.provinces.find(p => p.name === result.data.province);
                    if (province) {
                        showProvinceDetails(province.id);
                    }
                    searchResults.style.display = 'none';
...
function showProvinceDetails(provinceId) {
    ...
    document.getElementById('province-name').textContent = `Chi tiết ${province.name}`;
    document.getElementById('province-total').textContent = province.total_units;
    document.getElementById('province-merged').textContent = province.merged_units;
    document.getElementById('province-unchanged').textContent = province.unchanged_units;
    ...
    // Scroll to details
    document.getElementById('province-details').scrollIntoView({ behavior: 'smooth' });
